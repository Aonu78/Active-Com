...
class OrderController extends Controller
{
    ...
    public function show($id)
    {
        ...
        return view('seller.orders.show', compact('order', 'delivery_boys' , 'commission_percentage'));
    }
    public function verifyTransactionPin(Request $request)
    {
        ...
        if ($shop->transaction_password === $request->pin) {
            return response()->json(['status' => 'success']);
        }
        // If transaction_password is hashed (recommended):
        // if (Hash::check($request->pin, $shop->transaction_password)) {
        //     return response()->json(['status' => 'success']);
        // }
    ...
    public function update_payment_status(Request $request)
    {
        ...
        if ($request->status == 'paid') {
            if ($user->balance < $order->grand_total) {
                return response()->json([
                    'status' => 'error',
                    'message' => 'Insufficient balance to mark this order as paid.'
                ], 400);
            }
        }
        ...
        //  Deduct balance only if paid and passed earlier check
