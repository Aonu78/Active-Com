...
class CommissionController extends Controller
{
    ...
    public function pay_to_seller(Request $request)
    {
        ...
        $data['payment_decision'] = $request->payment_decision; 
    ...
    public function seller_payment_done($payment_data, $payment_details){
        ...
        if ($payment_data['payment_withdraw'] == 'withdraw_request') {
            ...
            if ($payment_data['payment_decision'] === 'reject') {
                $seller_withdraw_request->payment_decision = 'canceled';

                // Refund the amount to authenticated user's balance
                
                if ($shop->user) {
                    $shop->user->balance += $payment_data['amount'];  // assuming 'balance' is the field name
                    $shop->user->save();
                }
            } else {
                $seller_withdraw_request->payment_decision = 'paid';
            }
