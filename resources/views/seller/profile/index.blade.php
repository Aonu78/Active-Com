...
@section('panel_content')
    ...
    <form action="{{ route('seller.profile.update', $user->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <label class="col-md-3 col-form-label" for="usdt_address">{{ translate('USDT TRC20 Address') }}</label>
                    <div class="col-md-9">
                        <input type="text" name="usdt_address" value="{{ $user->shop->usdt_address }}" id="usdt_address" lang="en" class="form-control mb-3" placeholder="{{ translate('USDT TRC20 Address')}}">
                        @error('usdt_address')
                            <small class="form-text text-danger">{{ $message }}</small>
                        @enderror
                    </div>
    ...
    <form action="{{ route('user.transaction_password.update') }}" method="POST">
        @csrf
        <div class="card rounded-0 shadow-none border">
            <div class="card-header pt-4 border-bottom-0">
                <h5 class="mb-0 fs-18 fw-700 text-dark">{{ translate('Update Transaction Password') }}</h5>
            </div>
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-md-3">
                        <label class="fs-14">{{ translate('Your Login Password') }}</label>
                    </div>
                    <div class="col-md-9">
                        <input type="password" class="form-control rounded-0" name="login_password" required placeholder="{{ translate('Enter your current login password') }}">
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-md-3">
                        <label class="fs-14">{{ translate('New Transaction Password') }}</label>
                    </div>
                    <div class="col-md-9">
                        <input type="text" class="form-control rounded-0" name="transaction_password" required placeholder="{{ translate('Enter new transaction password') }}">
                    </div>
                </div>
                <div class="form-group text-right">
                    <button type="submit" class="btn btn-primary rounded-0 w-150px mt-3">{{ translate('Update Password') }}</button>
                </div>
            </div>
        </div>
    </form>
